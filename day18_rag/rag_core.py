...
def chat(messages: list[dict], use_cache: bool = True) -> tuple[str, int, int]:
    ...
    use_cache=False（Day 18 新增）：完全繞過快取，讀也不讀、寫也不寫。
    Day 18 要量「同一個 prompt 問五次，裁判會不會改口」——
    快取會讓這件事永遠量到 100% 一致，那是快取的一致性，不是模型的。
    這個參數只給裁判一致性實驗用；正式判定一律走快取，數字才可重現。
    """
    if use_cache:
