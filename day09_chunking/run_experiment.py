...
def run_strategy(name: str, chunk_func, text: str,
    ...
    vectors, index_tokens, _ = rag_core.get_embeddings([c["text"] for c in chunks])
    ...
    return {"name": name, "chunk_count": len(chunks), "avg_len": avg_len,
            ...
            "index_tokens": index_tokens, "question_tokens": question_tokens}
...
def main() -> None:
    ...
    # --- 總表：三種切法並排比較 ---
    lines += [
        "| 切法 | chunk 數 | 平均字數 | 沿用10題 | 其中直白 | 其中口語 | 新增5題 | 全部 |",
        "|------|---------|---------|---------|---------|---------|--------|------|",
    ]
